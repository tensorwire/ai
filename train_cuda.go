...
func cmdTrainCUDA() {
	...
	for step := 1; step <= *stepsFlag; step++ {
		...
		for li := range lays {
			...
			// QKV — overwrite pre-allocated buffers
			// MatMulTransposeBT returns new tensor — we need CopyInto
		...
		// Final RMSNorm backward — use dEmbed as scratch (same size [n,dim] fits)
